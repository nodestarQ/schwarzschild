<script lang="ts">
  ...
  import { callRelay } from "$lib/utils/relay";
  ...
  async function handleSendTransaction() {
    try {
      if (!burnAddress || !amount || !ephemeralPublicKey) {
...
<div class="space-y-4">
  ...
  {#if !transactionHash && !transactionError}
    <div class="space-y-3">
      ...
      <Alert>
        ...
        <AlertDescription>
          Click the Burn button below to submit this transaction. You'll be
          asked to confirm in your wallet.
        </AlertDescription>
      </Alert>
    </div>
  {/if}
  ...
  <div class="flex gap-2 pt-4">
    {#if transactionHash && !transactionError}
      <Button variant="outline" class="flex-1 gap-2" onclick={openExplorer}>
    ...
    {:else if transactionError}
      <Button variant="outline" class="flex-1" onclick={onBack}>Back</Button>
      <Button class="flex-1" onclick={handleRetry} disabled={isSending}>
        Try Again
      </Button>
    {:else}
      <Button
